...
const AdminDetails = () => {
    ...
    const handleDeleteUser = async (e) => {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            try {
              const response = await adminRequest.post(`${BASE_URL}/adminUser/delete`, {
                "email": `${activeURL}`
              });
              setRefresh(!refresh);
              Swal.fire({
    ...
    const handleBlockUser = async (e) => {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            try {
              const response = await adminRequest.post(`${BASE_URL}/adminUser/block`, {
                "email": `${activeURL}`
              });
              setRefresh(!refresh);
              Swal.fire({
            ...
            <div className="adminDetailsContents">
                ...
                <>
                    ...
                    <div className="bottom">
                        ...
                        <button type="button" className="bottom-btn" disabled={data.data.status.name === 'DELETED'} onClick={handleBlockUser}>{data.data.status.name === 'BLOCKED' ? 'Unblock' : 'Block'}</button>
